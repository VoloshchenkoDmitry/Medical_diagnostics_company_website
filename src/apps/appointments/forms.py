...
class AppointmentForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        self.fields["desired_date"].widget = forms.DateInput(
            attrs={"class": "form-control", "type": "date", "min": tomorrow.isoformat()}
        )
    ...
    def clean(self):
        ...
        if desired_date and desired_time:
            if Appointment.objects.filter(
                ...
                raise forms.ValidationError(_("Выбранное время уже занято. Пожалуйста, выберите другое время."))
