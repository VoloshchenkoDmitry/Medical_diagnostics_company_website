...
class Appointment(models.Model):
    ...
    desired_date = models.DateField(
        verbose_name=_('Желаемая дата')
    )
    desired_time = models.CharField(
        max_length=5,
        choices=TIME_SLOTS,
        verbose_name=_('Желаемое время')
    )
    ...
    patient_phone = models.CharField(
        max_length=20,
        verbose_name=_('Телефон пациента')
    )
    patient_email = models.EmailField(
        verbose_name=_('Email пациента')
    )
...
class AppointmentResult(models.Model):
    ...
    recommendations = models.TextField(
        verbose_name=_('Рекомендации')
    )
    prescription = models.TextField(
        blank=True,
        verbose_name=_('Назначения')
    )
